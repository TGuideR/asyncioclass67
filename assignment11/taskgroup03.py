    ...
    while not queue.empty():
        ...
        for product in customer.products:
            if cashier_number == 2:
                ...
                product.checkout_time += (0.1 * cashier_number) 
        ...
        product_time = round(time.perf_counter() - customer_start_time, ndigits=2)
        total_time += product_time
        print(f"The Cashier_{cashier_number} "
              f"finished checkout Customer_{customer.customer_id} "
              f"in {product_time} secs ")
...
async def main():
    ...
    print(f"\nThe supermarket process finished "
          ...
          f"in {round(time.perf_counter() - start_time, ndigits=2)} secs\n")
